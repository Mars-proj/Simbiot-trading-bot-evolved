...
class RNNModel:
    def __init__(self, lookback=20, units=64, dropout_rate=0.2):
    ...
    def _build_model(self):
        ...
        try:
            model = Sequential([
                SimpleRNN(self.units, input_shape=(self.lookback, 1), return_sequences=True),
                Dropout(self.dropout_rate),
                SimpleRNN(self.units // 2),
                Dropout(self.dropout_rate),
    ...
    def preprocess_data(self, klines):
        """Preprocess klines data for RNN."""
        try:
            if len(klines) < self.lookback:
                logger.warning("Not enough klines data for RNN preprocessing")
                return None, None
            ...
            y = np.array(y)
            logger.info(f"Preprocessed {len(X)} samples for RNN")
            return X, y
        except Exception as e:
            logger.error(f"Failed to preprocess data for RNN: {str(e)}")
            return None, None
